...
def relacion_categoricas(df, cat_col1, cat_col2, relative_freq=False, show_values=False, size_group = 5):
    # Prepara los datos
    ...
    if len(unique_categories) > size_group:
        ...
        for i in range(num_plots):
            ...
            ax = sns.barplot(x=cat_col1, y='count', hue=cat_col2, data=data_subset, order=categories_subset, palette = "viridis")
        ...
        plt.figure(figsize=(10, 6))
        ax = sns.barplot(x=cat_col1, y='count', hue=cat_col2, data=count_data, palette = "viridis")
...
def relacion_categorica_numerica(df, categorical_col, numerical_col, show_values=False, measure='mean'):
    ...
    # Si hay más de 5 categorías, las divide en grupos de 6
    if grouped_data.shape[0] > 6:
        unique_categories = grouped_data.index.unique()
        num_plots = int(np.ceil(len(unique_categories) / 6))

        for i in range(num_plots):
            # Selecciona un subconjunto de categorías para cada gráfico
            categories_subset = unique_categories[i * 6:(i + 1) * 6]
        ...
        # Crea el gráfico para menos de 6 categorías
        plt.figure(figsize=(10, 6))
...
def histograma_caja_bigotes(df, columns, whisker_width=1.5, bins = None):
...
def cajas_agrupadas(df, cat_col, num_col):
...
def histogramas_agrupados(df, cat_col, num_col, group_size):
    unique_cats = df[cat_col].unique()
    num_cats = len(unique_cats)
...
def dispersion_tres_var(df, col_x, col_y, col_size, scale = 1000):
    """
...
def dispersion_cuatro_var(df, x_col, y_col, color_col = None, size_col = "", scale = 1, legend = True):
    ...
    if type(size_col) == str:
        ...
        scatter = sns.scatterplot(x = x_col,
                                  ...
                                  sizes = sizes)
    if legend:
        ax.legend(ncol = 2, loc = "upper right", frameon = False, labelspacing = 3)
    ax.set_xlabel(x_col)
    ax.set_ylabel(y_col)
    plt.show()